...
static int uevent_event(struct powerhal_info *pInfo)
{
    ...
    if (strstr(cp, UEVENT_STRING)) {
        ALOGI("%s: uevent_event recieved: %s", __func__, cp);
    }
    return 0;
}

void *thread_uevent(void *pInfoPtr)
{
    struct powerhal_info *pInfo = pInfoPtr;
    while (1) {
        int nevents, ret;
...
static void uevent_init(struct powerhal_info *pInfo)
{
    ...
    pthread_create(&tid, NULL, thread_uevent, (void *)&pInfo);
